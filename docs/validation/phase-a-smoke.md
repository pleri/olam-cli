...
scrub-check: N matches (0 unaddressed) in plugin
  [allowed] scripts/scrub-allowlist.md:LINE — /<brand-token>/ ...
  [allowed] scripts/scrub-allowlist.md:LINE — /<kebab-token>/ ...
  [allowed] scripts/scrub-allowlist.md:LINE — /<handle-token>/ ...
  [allowed] scripts/scrub-allowlist.md:LINE — /<ticket-prefix>/ ...
  [allowed] scripts/scrub-denylist.json:LINE — /<brand-token>/ ...
  [allowed] scripts/scrub-denylist.json:LINE — /<kebab-token>/ ...
  [allowed] scripts/scrub-denylist.json:LINE — /<handle-token>/ ...
  [allowed] scripts/scrub-denylist.json:LINE — /<ticket-prefix>/ ...
```

(Output redacted to placeholders so the smoke doc itself does not embed the
literal denylist tokens and become self-falsifying. Run the command locally
to see the actual matches; all are self-referential tooling allowlisted in
`scripts/scrub-allowlist.md`.)

Exit code: 0. All matches are self-referential tooling (the scrub denylist
and allowlist necessarily contain the literal forbidden tokens); each is
documented in `scripts/scrub-allowlist.md`.
